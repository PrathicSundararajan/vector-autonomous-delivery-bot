        ...
        while True:
            ...
            while not m_confident:
                ...
                if len(markers) == 0:
                    ...
                    robot.behavior.turn_in_place(degrees(25))
                if m_confident:
                    print('confident')
            ...
            if not_at_goal:
                ...
                if GOAL_X - m_x > 0 and GOAL_Y - m_y > 0:
                    dist_ang = math.degrees(math.atan(dist_y/dist_x))
                elif GOAL_X - m_x < 0 and GOAL_Y - m_y > 0:
                    dist_ang = 180 - math.degrees(math.atan(dist_y/dist_x))
                elif GOAL_X - m_x < 0 and GOAL_Y - m_y < 0:
                    dist_ang = 180 + math.degrees(math.atan(dist_y/dist_x))
                elif GOAL_X - m_x > 0 and GOAL_Y - m_y < 0:
                    dist_ang = 360 - math.degrees(math.atan(dist_y/dist_x))
                else: 
                    dist_ang = 0
                print("turning in place now")
                robot.behavior.say_text("Turning")
                print(dist_ang)
                robot.behavior.turn_in_place(degrees(dist_ang))
                time.sleep(6)
                robot.behavior.say_text("Moving to goal")
                print("Moving straight towards goal now")
                robot.behavior.drive_straight(distance_mm(pythagorean_distance * 25.4),speed_mmps(100))
                time.sleep(10)
                robot.behavior.turn_in_place(GOAL_H)
                not_at_goal = False
